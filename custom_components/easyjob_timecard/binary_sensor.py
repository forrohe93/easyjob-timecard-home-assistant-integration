...
async def async_setup_entry(
    ...
    # Stable base for unique_id across reinstalls/migrations
    uid_base = entry.unique_id or entry.entry_id

    # --- CLEANUP: remove legacy + deselected dynamic entities from entity registry ---
    ent_reg = er.async_get(hass)

    # Legacy scheme (pre-migration): f"{entry.entry_id}_status_active_{status_id}"
    legacy_prefix = f"{entry.entry_id}_status_active_"

    # Current scheme (stable): f"{uid_base}__status_active_{status_id}"
    # (double underscore matches your current unique_id scheme for other entities too)
    current_prefix = f"{uid_base}__status_active_"
    ...
    for reg_entry in er.async_entries_for_config_entry(ent_reg, entry.entry_id):
        ...
        # 1) If we have a stable uid_base (i.e. migrated), remove legacy scheme entities.
        # These are the ones that tend to show up as duplicates + "Nicht verfügbar".
        if uid_base != entry.entry_id and reg_entry.unique_id.startswith(legacy_prefix):
            to_remove.append(reg_entry)
            continue

        # 2) Remove deselected dynamic entities from the current scheme.
        if reg_entry.unique_id.startswith(current_prefix):
            status_part = reg_entry.unique_id[len(current_prefix) :]
            try:
...
class EasyjobConnectedBinarySensor(_BaseEasyjobBinarySensor):
    ...
    def __init__(self, runtime: RuntimeData, entry: ConfigEntry) -> None:
        ...
        self._attr_unique_id = f"{self._uid_base}__connected"
...
class EasyjobWorktimeActiveBinarySensor(_BaseEasyjobBinarySensor):
    ...
    def __init__(self, runtime: RuntimeData, entry: ConfigEntry) -> None:
        super().__init__(runtime, entry)
        self._attr_unique_id = f"{self._uid_base}__worktime_active"
...
class EasyjobResourceStatusActiveBinarySensor(_BaseEasyjobBinarySensor):
    ...
    # Übersetzbarer Prefix + dynamischer Caption-Teil via placeholders
    # Erwartet in translations/strings.json:
    # entity.binary_sensor.status_active.name = "Status aktiv: {caption}"
    ...
    def __init__(
        ...
        self._attr_unique_id = f"{self._uid_base}__status_active_{self._status_id}"
